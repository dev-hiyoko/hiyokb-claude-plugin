...
（実装方針・調査・決定事項・進捗）を `~/.task` に蓄積して、**別の Claude セッションでも
...
初回にスキルを実行すると `~/.task/` 一式（`config.yaml` / `index.md` / `projects/` / `inbox/` / `sources/` / `kb/`）が作成されます。
...
## 設定（per-machine / `~/.task/config.yaml`）
...
機密を含みうる `config.yaml` / `inbox/` / `sources/` は `~/.task/.gitignore` で同期対象外にできます。
