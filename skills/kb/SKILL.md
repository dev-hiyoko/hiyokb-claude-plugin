...
## 追記（新規ページ or 既存改訂）
1. 行き先を決める。**既定は「今いるリポジトリのプロジェクト」の kb**（`project_bind.py --whoami` で解決し `projects/<project>/kb/` へ）。**複数プロジェクトに効く横断汎用な知識のときだけ** `~/.hiyokb/kb/`（global）に置く。どちらか迷うときはプロジェクト側に寄せる（後で横断昇格できる）。プロジェクト未設定なら所属を確認してから。
