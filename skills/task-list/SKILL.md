...
2. **絞り込んで表示**:
   ...
   - **「全プロジェクト」要求**: `--project` を付けずに横断表示:
     ...
     ```
   - **プロジェクト未設定（repo はあるが未登録）**: その旨を伝え「このリポジトリはどのプロジェクトの作業ですか？」を確認 → `project_bind.py github <repo> <プロジェクト名>` で登録してから絞り込む。確認できるまでは全プロジェクト表示にして、未設定である旨を明示。
   - **repo 無し（git 外）**: 横断表示。
   - 「完了も」のときは上記コマンドに `--all` を追加。
   - index が古い/空なら `/hiyokb:task-sync` を提案（勝手に同期しない。「最新で」と言われたら実行）。

## 表示のしかた（重要：作り直さない）
`build_index.py --stdout` は **状態 / ID / タイトル / プロジェクト / 優先度 / 期限 / 紐づき** の表＋凡例を既に出力する。
- **その出力（表・凡例）をそのまま提示する**。列を削らない・**リポジトリ別に作り直さない**・`#` とタイトルだけの簡易表に縮めない（情報が落ちて見にくくなる）。
- 並びは「状態（着手中→詰まり→レビュー→未着手）→優先度→期限」で既にソート済み。崩さない。
- 件数が多い（数十件）ときは、上位の着手中/優先度高を表で見せ、残りは「他 N 件（続き見ますか？）」と畳んでよい。**ただし列は減らさない**。
- 「プロジェクト」と「紐づき」は別軸。**プロジェクト=所属**、**紐づき=参照元との対応**（📄ドシエあり/⊘ソースのみ/🔗複数ソース）。混同しない。
