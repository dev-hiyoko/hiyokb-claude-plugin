...
## 表示する列（プロジェクト名を必ず出す）
表は **状態 / タスク / プロジェクト / 紐づき** を列に出す。`build_index.py` 出力の `project` 列をそのまま使う。
- **全プロジェクト表示（横断・プロジェクト未設定時）**: タスクごとに**プロジェクト名**を列で出す（空なら「（未分類）」）。
- **現プロジェクトに絞り込み済み**: 全行同じなので見出しに `プロジェクト: <名>` と書き、列は省略可。
- 「プロジェクト」と「紐づき」は別軸。**プロジェクト=所属**、**紐づき=参照元との対応状態**（📄ドシエあり/⊘ソースのみ/🔗複数ソースをリンク済）。混同しない。
